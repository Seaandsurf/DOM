...
for (let i = 0; i < children.length; i++) {
    ...
    // // (*) Remove duplicates
    // var uniqueFranchises = Array.from(new Set(Array.from(ul.children).map(child => child.textContent.trim())));
    // ul.innerHTML = '';
    ...
    //   if (event.key === 'r') {
    //     var franchises = Array.from(ul.children);
    ...
    //   if (event.key === 'd') {
    //     var fastAndFurious = document.querySelector('.important');
    //     var clone = fastAndFurious.cloneNode(true);
    ...
    //   var visibility = this.value === 'important franchises' ? 'visible' : 'hidden';
